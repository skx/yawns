...
sub delete_message
{
    ...
    #
    #  Get username and message ID.
    #
    my $username = $session->param("logged_in");
    my $id       = $form->param("delete_message");

    die "No username" unless defined($username);
    die "Invalid ID" unless ( $id =~ /^([0-9]+)$/ );
...
sub send_message
{
    ...
    #
    my $username  = $session->param("logged_in");
    my $recipient = $form->param("send_message");
    ...
    #
    #  Non-anonymous users can send messages to everybody if they
    # have special permissions, but only Steve if not.
    #
    my $perms = Yawns::Permissions->new( username => $username );

    #
    #  Admins can send to anybody
    #
    if ( !$perms->check( priv => "send_message" ) )
    {

        #
        #  If site-messaging is available all recipients are valid.
        #
        ...
        else
        {
            ...
            #
            #  Find each user who has "send_message" permission,
            # they are valid message recipients.
            #
            my @valid = $perms->findWith("send_message ");
            ...
            foreach my $u (@valid)
            {
                ...
                {
                    $found += 1
                      if ( lc( $target->{ 'username' } ) eq lc($recipient) );
                }
            }

            #
            #  No match?  Then the message is denied.
            #
            return ( permission_denied( admin_only => 1 ) ) if ( !$found );
        }
    }
    ...
    #
    #
    my $template = load_layout( "send_message.inc", session => 1 );

    #
    #  Has the message been sent?  Was there an error?
    #
    my $sent  = 0;
    my $empty = 0;
    ...
    #
    my $send = $form->param('send');
    if ( defined($send) && ( $send eq "send" ) )
    {
        ...
        #  Recipient.
        #
        my $u = Yawns::User->new( username => $recipient );
        my $d = $u->get();

        if ( !defined( $d->{ 'username' } ) )
        {
            return ( permission_denied( no_such_user => 1 ) );
        }
    ...
    $template->param( recipient => $recipient,
                      sent      => $sent,
                      empty     => $empty,
                      title     => "Send Message",
                    );
...
sub view_message
{
    ...
    #
    #  We can only view our own messages.
    #
    my $username = $session->param("logged_in");
    ...
    # open the html template
    my $template = load_layout( "view_message.inc",
                                session     => 1,
                                global_vars => 1
                              );
...
sub message_reply
{
    ...
    #  Get the message ID
    #
    my $id = $form->param("message_reply");
    die "Invalid ID" unless ( $id =~ /^([0-9]+)$/ );

    #
    #  Fetch the original message, to determine who we're replying to.
    #
    my $msg = Yawns::Messages->new( username => $username );
    my $message = $msg->getMessage($id);
    die "Invalid message" unless ( defined( @$message[0] ) );
    my $sender = @$message[0]->{ 'from' };
    ...
    #
    #
    my $template = load_layout( "message_reply.inc", session => 1 );

    #
    #  Are we sending a message?  Was there an error?
    #
    my $sent  = 0;
    my $empty = 0;

    if ( defined( $form->param("send") ) &&
         ( $form->param("send") eq "send" ) )
    {
        ...
        else
        {
            ...
            #
            print $form->redirect(
                         $protocol . $ENV{ "SERVER_NAME" } . "/view/messages" );
...
sub view_messages
{
    ...
    #
    my $msg = Yawns::Messages->new( username => $username );
    my $messages = $msg->getMessages();

    #
    #  Get message counts.
    #
    my ( $unread, $total ) = $msg->messageCounts();
    my $empty = 0;
    $empty = 1 if ( !defined($messages) );
    $empty = 1 if ( $total < 1 );

    # open the html template
    my $template = load_layout( "view_messages.inc",
                                session     => 1,
                                global_vars => 1
                              );
