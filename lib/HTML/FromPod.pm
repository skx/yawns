...
package HTML::FromPod;

=head1 NAME

HTML::FromPod - convert Perl POD documents to HTML
...
B<HTML::FromPod> converts one or more Pod documents into individual HTML
...
Optionally B<HTML::FromPod> can generate a table of contents and an index.
...
B<HTML::FromPod> class directly are marked with (*).

=over 4

=item B<-converter> I<module>

The converter class to use, defaults to C<HTML::FromPod>. This hook allows
...
sub pod2html
{
    ...
    # set defaults
    _default( \%opts, '-converter',  'HTML::FromPod' );
    ...
    my @cache =
      map {$_->[0]}
      sort {$a->[1] cmp $b->[1] || $a->[0]->page() cmp $b->[0]->page()}
    ...
    for ( my $i = 0 ; $i < scalar(@cache) ; $i++ )
    {
        ...
        my $outfile;
        my $outpath =
          _construct_file_name( $_->page(), 0, $opts{ -filesuffix } );
        if ( $opts{ -filter } )
        {
            $outfile = \*STDOUT;
        }
        ...
        if ( $opts{ -ps } )
        {
            my $pspath =
              $opts{ -psdir } . '/' .
              _construct_file_name( $_->page(), 0, '.ps' );
    ...
    if ( $opts{ -toc } )
    {
        ...
        # create a HTML::FromPod object to gain access to the customize
        # method
    ...
    if ( $opts{ -idx } )
    {
        ...
        foreach my $key ( qw(Sym 0-9), sort keys %idx )
        {
            ...
            foreach my $text ( sort {lc $a cmp lc $b} keys %{ $idx{ $key } } )
            {
                ...
                $idxobj->{ _body }
                  ->push_content( HTML::Element->new('br'), "\n" );
            }
            delete $idx{ $key };
        }
...
The B<HTML::FromPod> module has an object oriented interface that allows
...
This method also reads properties of the current HTML::FromPod object
...
customization by writing a new module that inherits from B<HTML::FromPod>:

  package POD::HTML::mystyle;
  use HTML::FromPod qw(pod2html);
...
customize method from B<HTML::FromPod>.
...
sub customize
{
    ...
    # set document class
    my $root = HTML::Element->new( '~declaration',
    ...
    if ( $self->{ -navigation } )
    {
        ...
        if ( $self->{ '-next' } )
        {
            ...
            my $anchor =
              HTML::Element->new( 'a',
                                  CLASS => 'POD_NAVLINK',
                                  href =>
                                    _construct_file_name( $self->{ '-next' },
                                                          $self->depth(),
                                                          $self->{ -suffix } )
                                );
            $anchor->push_content( 'Next:',
                                   HTML::Element->new('br'),
                                   $self->{ '-next' } );
        ...
        if ( $self->{ '-prev' } )
        {
            ...
            my $anchor =
              HTML::Element->new( 'a',
                                  CLASS => 'POD_NAVLINK',
                                  href =>
                                    _construct_file_name( $self->{ '-prev' },
                                                          $self->depth(),
                                                          $self->{ -suffix } )
                                );
            $anchor->push_content( 'Previous:',
                                   HTML::Element->new('br'),
                                   $self->{ '-prev' } );
            $td->push_content($anchor);
            $tr->push_content($td);
        ...
        {
            my $td =
              HTML::Element->new( 'td', align => 'right', width => '1%' );
            my $anchor =
              HTML::Element->new( 'a',
                                  CLASS => 'POD_NAVLILNK',
                                  href =>
                                    _construct_file_name( $self->{ -tocname },
                                                          $self->depth(),
                                                          $self->{ -suffix } )
                                );
        ...
        if ( $self->{ -idx } )
        {
            ...
            my $anchor =
              HTML::Element->new( 'a',
                                  CLASS => 'POD_NAVLINK',
                                  href =>
                                    _construct_file_name( $self->{ -idxname },
                                                          $self->depth(),
                                                          $self->{ -suffix } )
                                );
    ...
    $anchor = HTML::Element->new( 'a',
                                  CLASS => 'POD_NAVLINK',
                                  href  => '#Pod_TOP_OF_PAGE'
                                );
    $anchor->push_content('[Top]');
    $self->{ _body }
      ->push_content( HTML::Element->new('hr'), "\n", $anchor, "\n" );
    $self->{ _body }->push_content(
                "Generated by HTML::FromPod $VERSION on " . localtime() . "\n" )
      if ( $self->{ -banner } );
}
...
sub begin_input
{
    ...
    ( $self->{ _html }, $self->{ _head }, $self->{ _body } ) = _basic_html();
...
sub end_pod
{
    ...
    # dump it
    _write_html( $self->{ _html }, $self->output_file(),
                 $out_fh,          $self->{ -verbose } );
    1;
}
...
sub command
{
    ...
    if ( $command =~ /^head(\d)/ )
    {
        ...
        my $heading =
          HTML::Element->new( 'h' . ( $n + 1 ), CLASS => "POD_HEAD$n" );
        ...
        {
            push( @{ $self->{ _toc } },
                  [$n, $id, HTML::Element->clone_list(@title)] );
        }
    }
    ...
    elsif ( $command eq 'item' )
    {
        ...
        unless ( $list->type() )
        {
            ...
            $list->tag(
                      HTML::Element->new( $list->type(), CLASS => 'POD_LIST' ) )
              ->push_content("\n");
    ...
    elsif ( !$self->{ _begin } )
    {
        ...
        foreach ( split( /\n/, $paragraph ) )
        {
            ...
            if (s/^(\s+)([\w:]+)(\t+)//)
            {
                ...
                {
                    my $destfile =
                      _construct_file_name( $dest->page(), $self->depth(),
                                            $self->{ -suffix } );
                    my $link =
                      HTML::Element->new( 'a',
                                          href  => $destfile,
...
sub interpolate
{
    ...
    $self->_expand_ptree( $self->parse_text( $paragraph, $line ), $line, '' );
}
...
sub _expand_ptree
{
    ...
    foreach (@$ptree)
    {
        ...
        elsif ( $cmd eq 'L' )
        {
            ...
            if ($page)
            {
                ...
                if ($dest)
                {
                    $destfile =
                      _construct_file_name( $dest->page(), $self->depth(),
                                            $self->{ -suffix } );
                    $self->{ _link_pagemark } = $self->{ _link_sectionmark } =
                      'a';
                    $self->{ _link_pageopt } = $self->{ _link_sectionopt } =
            ...
            else
            {
                ...
                if ( $link->node() )
                {
                    ...
                    $node =
                      $cruncher->interpolate_and_check( $link->node(), $line,
                                                        $file );
                }
                if ( $dest && $node )
                {
                    ...
                    else
                    {
                        ...
                        warn
                          "Cannot find node `$node'$inpage at L<> on line $line\n";
                    }
                }
            }
            ...
            push( @text,
                  $self->_expand_ptree(
                                        $self->parse_text($link->markup(), $line
                                                         ),
                                        $line,
                                        "$nestlist$cmd"
                                      ) );
        ...
        elsif ( $cmd eq 'P' )
        {
            ...
            push( @text, $tag );
            $tag->push_content(
                    $self->_expand_ptree( $contents, $line, "$nestlist$cmd" ) );
        }
...
sub _autolink_and_highlight
{
    ...
    $cruncher->errorsub( sub {1;} );    # suppress any errors
    my $text =
      $cruncher->interpolate_and_check( $contents->raw_text(), $line, '' );
    $text =~ s/^\s+|\s+$//g;
            ...
            $$node_ref[1] eq $self->{ _current_anchor } ) )
    {
        my $anchor = HTML::Element->new(
                                    'a',
                                    CLASS => 'POD_LINK',
                                    href  => _construct_file_name(
                                        $$node_ref[0], $self->depth(),
        ...
        foreach ( $node->content_refs_list )
        {
            if ( ref $$_ && $$_->tag() eq 'p' )
            {
                ...
                if (@rest)
                {
                    ...
                    my $newp =
                      HTML::Element->new( 'p', $node->all_external_attr() );
