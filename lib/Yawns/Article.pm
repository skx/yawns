...
sub delete
{
    ...
    #
    $query = "DELETE FROM related WHERE article=?";
...
sub addRelated
{
    ...
    #
    my $sql =
      $db->prepare('INSERT INTO related (article,title,link) VALUES(?, ?, ?)');
...
sub deleteRelated
{
    my ( $class, $article, $id ) = (@_);

    die "No article" if not defined($article);
    die "No id"      if not defined($id);
...
sub getRelated
{
    ...
    #
    #  Anonymous users never will.  Otherwise check
    #
    if ( $username !~ /^anonymous$/i )
    {
        my $perms = Yawns::Permissions->new( username => $username );
        $related_admin = 1 if ( $perms->check( priv => "related_admin" ) );
    }
    ...
    # get required data
    my $sql = $db->prepare(
        'SELECT title,link,article,id FROM related WHERE article = ? ORDER BY title'
    );
